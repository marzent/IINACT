...
namespace RainbowMage.OverlayPlugin.MemoryProcessors {
    public class EnmityMemory62 : EnmityMemory {
        ...
        public EnmityMemory62(TinyIoCContainer container) {
        ...
        private bool GetPointerAddress() {
            ...
            if (!success) {
                if (loggedScanErrors < 10) {
                    logger.Log(LogLevel.Error, "Failed to find enmity memory for 6.2: {0}.", String.Join(",", fail));
                    loggedScanErrors++;
                ...
                logger.Log(LogLevel.Info, "Found enmity memory for 6.2.");
                loggedScanErrors = 0;
            }
