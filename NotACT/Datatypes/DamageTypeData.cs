namespace Advanced_Combat_Tracker {
    public class DamageTypeData : IEquatable<DamageTypeData>, IComparable<DamageTypeData> {
        ...
        public DateTime StartTime => Items.TryGetValue(ActGlobals.Trans["attackTypeTerm-all"], out var value) ? value.StartTime : DateTime.MaxValue;

        public DateTime EndTime => Items.TryGetValue(ActGlobals.Trans["attackTypeTerm-all"], out var value) ? value.EndTime : DateTime.MinValue;
        ...
        public TimeSpan Duration {
            get {
                if (Parent != null && Parent.Parent.StartTimes.Count > 1) {
                    ...
                    for (var i = 0; i < Parent.Parent.StartTimes.Count; i++) {
                        ...
                        for (var j = num; j < value.Items.Count; j++) {
                            if (list.Count == list2.Count) {
                                if (i == Parent.Parent.StartTimes.Count - 1 && Parent.Parent.EndTimes.Count + 1 == Parent.Parent.StartTimes.Count) {
                                    if (value.Items[j].Time >= Parent.Parent.StartTimes[i] && value.Items[j].Time <= Parent.Parent.EndTime) {
                                        list.Add(value.Items[j].Time);
                                        num = j;
                                    }
                                } else if (value.Items[j].Time >= Parent.Parent.StartTimes[i] && value.Items[j].Time <= Parent.Parent.EndTimes[i]) {
                                    list.Add(value.Items[j].Time);
                                    num = j;
                                }
                            }
                            if (list.Count - 1 == list2.Count) {
                                ...
                                for (var k = j; k < value.Items.Count; k++) {
                                    ...
                                    if (Parent.Parent.StartTimes.Count > i + 1 && value.Items[k + 1].Time >= Parent.Parent.StartTimes[i + 1]) {
                                        num = k - 1;
                                        break;
                                    }
                                }
                    ...
                    if (list.Count != list2.Count) {
                        throw new Exception(string.Format("Personal Duration failure.  StartTimes: {0}/{2} EndTimes: {1}/{3}", list.Count, list2.Count, Parent.Parent.StartTimes.Count, Parent.Parent.EndTimes.Count));
                    }
        ...
        public float AverageDelay => Items.ContainsKey(ActGlobals.Trans["attackTypeTerm-all"]) ? Items[ActGlobals.Trans["attackTypeTerm-all"]].AverageDelay : float.NaN;

        public long Damage => Items.TryGetValue(ActGlobals.Trans["attackTypeTerm-all"], out var value) ? value.Damage : 0L;

        public int Swings => Items.TryGetValue(ActGlobals.Trans["attackTypeTerm-all"], out var value) ? value.Swings : 0;
        ...
        public float CritPerc => Items.TryGetValue(ActGlobals.Trans["attackTypeTerm-all"], out var value) ? value.CritPerc : 0f;

        public int Misses => Items.TryGetValue(ActGlobals.Trans["attackTypeTerm-all"], out var value) ? value.Misses : 0;

        public int Blocked => Items.TryGetValue(ActGlobals.Trans["attackTypeTerm-all"], out var value) ? value.Blocked : 0;
        ...
        public long Median => Items.TryGetValue(ActGlobals.Trans["attackTypeTerm-all"], out var value) ? value.Median : 0L;

        public long MinHit => Items.TryGetValue(ActGlobals.Trans["attackTypeTerm-all"], out var value) ? value.MinHit : 0L;

        public long MaxHit => Items.TryGetValue(ActGlobals.Trans["attackTypeTerm-all"], out var value) ? value.MaxHit : 0L;

        public double EncDPS => Items.TryGetValue(ActGlobals.Trans["attackTypeTerm-all"], out var value) ? value.EncDPS : 0.0;

        public double ExtDPS => EncDPS;

        public double CharDPS => Items.TryGetValue(ActGlobals.Trans["attackTypeTerm-all"], out var value) ? value.CharDPS : 0.0;
        ...
        public override int GetHashCode() => Items.Values.Aggregate(0L, (current, t) => current + t.GetHashCode()).GetHashCode();
