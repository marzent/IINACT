...
namespace RainbowMage.OverlayPlugin.EventSources {
    public class CactbotEventSource : EventSourceBase {
        ...
        private JObject FetchUserFiles(JObject msg) {
            ...
            GetUserConfigDirAndFiles(msg["source"].ToString(), out var config_dir, out user_files);
        ...
        private struct OverlayPreset : IOverlayPreset {
            public string Name { get; set; }
            public string Type => "MiniParse";
            public string Url { get; set; }
            public int[] Size { get; set; }
            public bool Locked { get; set; }
            public List<string> Supports => new List<string> { "modern" };
        }

        private void RegisterPreset(string dirName, int width, int height, string nameOverride = null, string fileOverride = null) {
            var lc = dirName.ToLowerInvariant();
