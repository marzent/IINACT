        ...
        public override void HandleEvent(JObject e) {
            if (Config.ActwsCompatibility) {
                ...
                switch (e["type"].ToString()) {
                    case "CombatData":
                        ((IOverlay)this).ExecuteScript("__OverlayPlugin_ws_faker({'type': 'broadcast', 'msgtype': 'CombatData', 'msg': " + e.ToString(Formatting.None) + " });");
                        break;
                    case "LogLine":
                        ((IOverlay)this).ExecuteScript("__OverlayPlugin_ws_faker({'type': 'broadcast', 'msgtype': 'Chat', 'msg': " + JsonConvert.SerializeObject(e["rawLine"].ToString()) + " });");
                        break;
                    case "ChangeZone":
                        ((IOverlay)this).ExecuteScript("__OverlayPlugin_ws_faker({'type': 'broadcast', 'msgtype': 'ChangeZone', 'msg': " + e.ToString(Formatting.None) + " });");
                        break;
                    case "ChangePrimaryPlayer":
                        ((IOverlay)this).ExecuteScript("__OverlayPlugin_ws_faker({'type': 'broadcast', 'msgtype': 'SendCharName', 'msg': " + e.ToString(Formatting.None) + " });");
                        break;
                }
            } else if (ModernApi) {
                base.HandleEvent(e);
                ...
                switch (e["type"].ToString()) {
                    case "CombatData":
                        ((IOverlay)this).ExecuteScript("document.dispatchEvent(new CustomEvent('onOverlayDataUpdate', { detail: " + e.ToString(Formatting.None) + " }));");
                        break;
                    case "LogLine":
                        ((IOverlay)this).ExecuteScript("document.dispatchEvent(new CustomEvent('onLogLine', { detail: " + JsonConvert.SerializeObject(e["line"].ToString(Formatting.None)) + " }));");
                        break;
                }
            }
        }
