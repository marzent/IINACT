namespace Advanced_Combat_Tracker {
	public class HistoryRecord : IComparable<HistoryRecord>, IEquatable<HistoryRecord> {
		...
		public override string ToString() {
			...
			var text = ((!(timeSpan.TotalHours > 1.0)) ? $"{timeSpan.Minutes:00}:{timeSpan.Seconds:00}" : $"{(int)timeSpan.TotalHours:00}:{timeSpan.Minutes:00}:{timeSpan.Seconds:00}");
			var text2 = ((Type != 1) ? string.Empty : "     ");
			var text3 = ((ActGlobals.oFormActMain.LogFilePath.ToLower() != FolderHint.ToLower()) ? FolderHint : string.Empty);
